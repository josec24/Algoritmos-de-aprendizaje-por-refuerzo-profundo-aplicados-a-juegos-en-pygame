        ...
        for steps in range(max_steps):
            ...
            if done or steps == max_steps-1:
                ...
                if score > record:
                    record = score
                    # print('record:',record)
                    agent.actor_critic.save('pacman_actorC_model.pth')
                
                guardarPuntuacion(score,game.getTime()-timeP,'datos_pacman_actorC.csv')
                timeP=game.getTime()
                game.reinicio()                
...
class A2cAgent:
    def __init__(self, env, learning_rate=3e-4, gamma=0.99):
        ...
        #Cargando el modelo
        self.actor_critic.load_state_dict(torch.load("./model/pacman_actorC_model.pth"))
