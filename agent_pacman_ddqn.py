...
from juegos.pacman.game import PacmanGame
        ...
        for step in range(max_steps):
            ...
            if done or step == max_steps-1:
                ...
                if score > record:
                    record = score
                    # print('record:',record)
                    agent.model.save('pacman_ddqn_model.pth')
                    agent.target_model.save('pacman_ddqn_target_model.pth')
                
                guardarPuntuacion(score,game.getTime()-timeP,'datos_pacman_ddqn.csv')
                timeP=game.getTime()
                game.getTime()
...
class DDQNAgent:
    def __init__(self, env, learning_rate=3e-4, gamma=0.99, tau=0.01, buffer_size=10000):
        ...
        #Cargando el modelo
        self.model.load_state_dict(torch.load("./model/pacman_ddqn_model.pth"))
        self.model.eval()
        self.target_model.load_state_dict(torch.load("./model/pacman_ddqn_target_model.pth"))
