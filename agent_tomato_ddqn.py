...
from juegos.tomatoplus.game import TomatoPlusGame
...
def train(game, agent, max_episodes, max_steps, batch_size):
    ...
    for episode in range(max_episodes):
        ...
        for step in range(max_steps):
            ...
            #Actualizando el batch del si sobrepasa el tamaño
            ...
            if done or step == max_steps-1:
                ...
                if score > record:
                    record = score
                    # print('record:',record)
                    agent.model.save('tomatoPlus_ddqn_model.pth')
                    agent.target_model.save('tomatoPlus_ddqn_target_model.pth')
                
                guardarPuntuacion(score,game.getTime()-timeP,'datos_tomatoPlus_ddqn.csv')
                timeP=game.getTime()
...
class DDQNAgent:
    def __init__(self, env, learning_rate=3e-4, gamma=0.99, tau=0.01, buffer_size=10000):
        ...
        # Cargando el modelo
        self.model.load_state_dict(torch.load("./model/tomatoPlus_ddqn_model.pth"))
        self.model.eval()
        self.target_model.load_state_dict(torch.load("./model/tomatoPlus_ddqn_target_model.pth"))
